    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(kecheng !=null){
            ...
            BeanUtils.copyProperties( kecheng , view );//把实体数据重构到view中
                ...
                //级联表
                JiangshiEntity jiangshi = jiangshiService.selectById(kecheng.getJiangshiId());
                if(jiangshi != null){
                    BeanUtils.copyProperties( jiangshi , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
    ...
    */
    ...
    public R save(@RequestBody KechengEntity kecheng, HttpServletRequest request){
        ...
        if(false)
            ...
            kecheng.setJiangshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<KechengEntity> queryWrapper = new EntityWrapper<KechengEntity>()
            ...
            .eq("jiangshi_id", kecheng.getJiangshiId())
            .eq("shangpin_clicknum", kecheng.getShangpinClicknum())
            ;
...
//                            kechengEntity.setKechengFile(data.get(0));                    //课程资料 要改的
//                            kechengEntity.setKechengTypes(Integer.valueOf(data.get(0)));   //课程类型 要改的
//                            kechengEntity.setJiangshiId(Integer.valueOf(data.get(0)));   //讲师 要改的
//                            kechengEntity.setInsertTime(date);//时间
//                            kechengEntity.setShangpinClicknum(Integer.valueOf(data.get(0)));   //点击次数 要改的
